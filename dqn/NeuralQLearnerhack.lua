...
local nql = torch.class('dqn.NeuralQLearnerhack')
...
function nql:getQUpdate(args)
    ...
    local soft = ss:forward(self.network.output)
    local hack_target = soft:clone():zero()
    local hack_err = mse:forward(soft, hack_target)
    local hack_grad = mse:backward(soft, hack_target)
    local hack_softgrad = ss:backward(self.network.output, hack_grad)
    self.network:backward(s, hack_softgrad)
